...
if not os.path.exists("Database_preprocessed"):
  os.makedirs("Database_preprocessed")

for i in range(0,len(second_level)):
        name = second_level[i].split("Database/",1)[1]
        if name.rsplit('/', 1)[-1] != ".DS_Store" and name.rsplit('/', 1)[-1] != "":
          if not os.path.exists("Database_preprocessed/"+name.rsplit('/',1)[0]):
            os.makedirs("Database_preprocessed/"+name.rsplit('/',1)[0])
          try:
            ...
            for k, d in enumerate(dets):
              ...
              CropFace(img, eye_left=left_eye, eye_right=right_eye, offset_pct=(0.20,0.20), dest_sz=(144,144)).save("Database_preprocessed/"+name)
          except:
            pass

script2 = """
for name in Database_preprocessed/*/*.jpg; do
  convert -colorspace Gray $name $name
done
"""
os.system("bash -c '%s'" % script2)
